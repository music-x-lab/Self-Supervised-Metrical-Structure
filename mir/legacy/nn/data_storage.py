...
class FramedH5DataStorage(BasicH5DataStorage,FramedStorage):
    ...
    def get_length(self):
        if(not self.created):
            raise Exception('No dataset created')
        if(self.total_song_count==-1):
            self.length=np.array(self.root['length'])
            self.start=np.array(self.root['start'])
