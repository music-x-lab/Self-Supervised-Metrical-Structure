...
import osu_parser.slidercalc as slidercalc
...
class BeatmapParser():
    ...
    def parse_timing_point(self, line):
        ...
        timing_point = {
            ...
            "timingChange": (int(members[6]) == 1),
            "kiaiTimeActive": (int(members[7]) == 1)
        }
    ...
    def parse_hit_object(self, line):
        ...
        elif(object_type & 128):
            # Mania Slider
            self.beatmap["nbSliders"] += 1
            hit_object["object_name"] = 'mania_slider'
            hit_object["end_time"] = int(members[5].split(':')[0])
            if len(members) > 7:
                hit_object["additions"] = self.parse_additions(members[6])
    ...
    def parse_event(self, line):
        ...
        if members[0] == '0' and members[1] == '0' and members[2]:
            bg_name = members[2].strip() # fixed

            if bg_name[0] == '"' and bg_name[len(bg_name) - 1] == '"':
                self.beatmap["bg_filename"] = bg_name[1: -1]
            else:
    ...
    def build_beatmap(self):
        ...
        self.beatmap["breakTimes"].sort(key=lambda a: a["startTime"])
